...
if [ ! -e /tmp/nomad-test.pid ]; then
    nomad agent -dev -bind 0.0.0.0 -acl-enabled -vault-address=$VAULT_ADDR -vault-token $VAULT_TEST_TOKEN -vault-enabled -vault-allow-unauthenticated=false > /dev/null 2>&1 &
    NOMAD_PID=$!
    ...
    nomad node status
    http --ignore-stdin POST http://0.0.0.0:4646/v1/acl/bootstrap | jq -r '.SecretID' > /tmp/nomad-test.token
